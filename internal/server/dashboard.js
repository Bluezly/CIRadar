...
function analysisRow(item){return `<tr data-open="analysis" data-id="${esc(item.id)}"><td>${fmtDate(item.created_at)}</td><td><b>${esc(item.repository||'local')}</b><small>${esc(item.workflow||'')} · ${esc(item.job||'')}</small></td><td>${pill(item.attribution)}<small>${esc(item.category)}</small></td><td class="summary"><b>${esc(item.summary)}</b><small>${esc(item.decision_reason||'')}</small></td><td>${scoreBar(item.score)}</td><td class="actions">${button('feedback-correct',item.id,'✓')}${button('feedback-partial',item.id,'~')}${button('feedback-incorrect',item.id,'✕','danger')}</td></tr>`}
function testRow(item){return `<tr data-open="test" data-id="${esc(item.test_key)}"><td><b>${esc(item.name)}</b><small>${esc([item.file,item.suite,item.class_name,item.parameters].filter(Boolean).join(' · '))}</small></td><td>${esc(item.repository)}</td><td>${Number(item.total_runs||0)}<small>${Number(item.passes||0)} pass · ${Number(item.failures||0)} fail</small></td><td>${scoreBar(item.flake_score)}</td><td>${esc(item.primary_flake_cause||'unknown')}<small>${(Number(item.cause_confidence||0)*100).toFixed(0)}% confidence</small></td><td>${pill(item.classification)}${item.quarantined?'<small>quarantined</small>':''}</td><td class="actions">${item.quarantined?button('unquarantine',item.test_key,'Restore'):button('quarantine',item.test_key,'Quarantine')}</td></tr>`}
...
function analysisDetail(item){
 ...
 return `${keyValues([['Repository',item.repository],['Workflow',item.workflow],['Job',item.job],['Provider',item.provider],['Category',item.category],['Attribution',item.attribution],['Score',item.score],['Fingerprint',item.fingerprint],['Created',fmtDate(item.created_at)]])}<h3>Evidence</h3><ul class="evidence">${evidence}</ul><h3>Decision</h3><p>${esc(item.decision_reason||item.summary)}</p><h3>Redacted excerpt</h3><pre>${esc(item.excerpt||item.redacted_excerpt||'Raw logs are not stored by default.')}</pre><h3>Recommended actions</h3>${actions}`
}
...
function testDetail(item){return `${keyValues([['Repository',item.repository],['Framework',item.framework],['File',item.file],['Suite',item.suite],['Class',item.class_name],['Runs',item.total_runs],['Passes',item.passes],['Failures',item.failures],['Flake score',item.flake_score],['Classification',item.classification],['Likely cause',item.primary_flake_cause],['Cause confidence',(Number(item.cause_confidence||0)*100).toFixed(0)+'%'],['Last seen',fmtDate(item.last_seen)]])}<h3>Quarantine</h3><p>${item.quarantined?'This test is currently quarantined.':'This test blocks CI when it fails.'}</p><div class="drawer-actions">${item.quarantined?button('unquarantine',item.test_key,'Restore'):button('quarantine',item.test_key,'Quarantine')}</div>`}
